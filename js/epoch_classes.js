...
function Epoch(name,mode,targetelement,multiselect) {
	...
	*/
	function calConfig() {
		...
		self.minDate = new Date(2005,1,1);
	...
	*/
	function setLang() {
		self.daylist = new Array('Dom','Lun','Mar','Mie','Jue','Vie','Sab','Dom','Lun','Mar','Mie','Jue','Vie','Sab');
		self.months_sh = new Array('Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic');
		self.monthup_title = 'Ir al mes próximo';
		self.monthdn_title = 'Ir al mes anterior';
		self.clearbtn_caption = 'Limpiar';
		self.clearbtn_title = 'Despeja cualquier fecha seleccionada en el calendario';
		self.maxrange_caption = 'Ésta es la gama máxima';
	...
	*/
	self.setTarget = function (targetelement, focus)
	{
		...
		if(self.mode == 'popup') {
			...
			self.leftOffset = -105; 					// the horizontal distance (in pixels) to display the calendar from the Left of its input element
	...
	*/
	self.updatePos = function (target) {
		if(self.mode == 'popup') {
			self.calendar.style.top = getTop(target) + self.topOffset + 'px'; // aca estoy cambienado
			self.calendar.style.left = getLeft(target) + self.leftOffset + 'px'; // aca estoy cambiando
		}
	};
	//-----------------------------------------------------------------------------
	/**
